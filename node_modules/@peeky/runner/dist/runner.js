...
export async function setupRunner(options) {
    const ctx = {
        options,
    };
    const { reporters } = options;
    const serializableConfig = toSerializableConfig(options.config);
    const { onMessage, clearOnMessage, handleMessage, } = useWorkerMessages();
    ...
    async function runTestFileWorker(options) {
        const suiteMap = {};
        const { mainPort, workerPort } = createWorkerChannel({
            ...
            onTestError: (suiteId, testId, duration, error) => {
                ...
                const [, test] = suite.children.find(([, t]) => t.id === testId);
                Object.assign(test, {
                    duration,
                    error,
                });
    ...
    async function runTestFile(relativePath, clearDeps = [], updateSnapshots = false) {
