...
export function createMockedFileSystem() {
    ...
    _memfs = memfs;
    const allFss = [realFs, memfs];
    const readFss = [realFs, memfs];
    const writeFss = [memfs];
    const ufs = {
        _enabled: false,
    };
    // Sync
    for (const [fsList, methods] of [[readFss, fsSyncMethodsRead], [writeFss, fsSyncMethodsWrite]]) {
    ...
    // Async
    for (const [fsList, methods] of [[readFss, fsAsyncMethodsRead], [writeFss, fsAsyncMethodsWrite]]) {
        for (const method of methods) {
            if (specialMethods.includes(method))
                continue;
            ufs[method] = (...args) => {
    ...
    // Promises
    ufs.promises = {};
    for (const [fsList, methods] of [[readFss, fsPromiseMethodsRead], [writeFss, fsPromiseMethodsWrite]]) {
        for (const method of methods) {
            if (specialMethods.includes(method))
                continue;
...
function createFSWatcherProxy(watchers) {
    return new Proxy({}, {
        get(_obj, property) {
            const funcCallers = [];
            let prop;
