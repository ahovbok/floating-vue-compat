...
export async function runTests(ctx, failOnSnapshots) {
    for (const suite of ctx.suites) {
        await runSuite(ctx, suite, failOnSnapshots);
    }
}
async function runSuite(ctx, suite, failOnSnapshots) {
    ...
    const ranTestChildren = suite.childrenToRun.filter(child => child[0] === 'test');
...
function getSuitesFromRootToLeaf(suite) {
